...
4. Wire **Messenger → Ollama** in the scene: on `ChatManager`, assign **`OllamaConnector`**; on `OllamaConnector`, assign **`ChatManager`** and (optionally) **`DeliveryManager`** so the hidden LLM **`[CONTEXT: …]`** matches delivery progress, **active drop-off id**, **apartment room** (ids **0–6** → rooms **201–208** when in the project table), and **pickup state** when a reception **`DeliveryItem`** is used. Optionally assign **`Typing Indicator Text`** on `ChatManager` for a pulsing “H is typing…” label (otherwise a temporary line is appended to the feed). Run **Ollama** with the model named in `ollama-plan.md`, then press **Play** and send a chat message to trigger a generate call. Model replies appear as **`[H]: …`** in the messenger. **Delivery flow (default):** the opening **H** intro does **not** assign a job; the **first player SEND** runs `PrepareNextDeliveryFromAi` and may append a scripted **H** line about the lobby package (`ChatManager` fields). **Reception package:** player must **Interact** (raycast) on the **`DeliveryItem`** before **`DeliveryZone`** accepts drop-off when that item is assigned on **`DeliveryManager`**. On **`ComputerDesktopUI`**, assign **`Shutdown Computer Button`** (or re-run **GameObject → Computer Desktop Canvas** to regenerate the prefab with a top-right **Shutdown** control) and **`Computer Terminal`** so shutdown exits the session. **HUD:** **GameObject → UI → Global Notification HUD (persistent)** creates overlay + `DeliveryManager`; **Repair Package Delivered Label (add TMP)** fixes a row that only had an `Image` (TMP must live on a child — one **Graphic** per GameObject). World use: put **`ComputerTerminal`** (and optionally **`ComputerInteract`**) on the computer object the player looks at; **Interact** (often **E**) raycasts via **`PlayerController`**.
