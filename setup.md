...
- **Scene restart:** `DeliveryManager` lives on the same **DontDestroyOnLoad** HUD as this timer — **`ResetRunStateForNewPlaySession`** runs from **`DeliveryUrgencyTimer`** on gameplay (and menu) load so **pause → Restart** resets **`currentDeliveryID`** and restores the **first-leg** budget (**e.g. 90s**). If **`prepareFirstDeliveryAfterSceneTick`** is on, the one-frame first-leg prepare is queued from that load path (not only from **`DeliveryManager.Start`**, which does not run again on a persisted object).
