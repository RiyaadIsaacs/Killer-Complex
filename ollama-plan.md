...
- **Session reload (DDOL):** **`GlobalNotificationHud`** survives scene loads; duplicate scene HUD instances **merge** package/spawn bindings into the persistent **`DeliveryManager`** before destroy. **`DeliveryZone`** registers drop points on the persistent manager. **`GameplaySessionReset`** + **`OllamaConnector.ResetSessionStateForSceneLoad`** + **`ChatManager` / `OllamaConnector` runtime resolve** rebind scene **`ChatManager`** and clear stale connector refs. **`GlobalNotificationHud.ResetTransientNotificationsForSession`** clears stale delivery toasts.
- **Messenger display:** Visible lines use **`ChatManager`** **`FormatLine`** — **bold sender** + **`<noparse>`** body (not raw `[H]:` in TMP). **`OllamaConnector`** strips echoed **`[CONTEXT]`** / quest-log prefixes and fixes **`my wife` → `your wife`** slips before **`UpdateChatFeed("H", …)`**.

---

## 5. Prompt structure (summary)

- **System prompt:** Hardcoded in `OllamaConnector` — persona **H** as a **cold kidnapper** with the player’s **wife** held hostage (fiction), **security-camera** surveillance, **orders not jokes**, **clinical** detail on the wife when the player **delays or fails** deliveries, **bru** / **wena** dominance; **hostage pronouns** (**your wife**, never **my wife**); **greeting/small-talk** rules (no “is everything ok”); **MESSENGER UI** rule (no `Job:` / `[H]:` prefixes — UI labels sender); never apologize; rude player → **concrete hostage threat**. Concise replies; fiction-only / no real PII; honour **`[CONTEXT: …]`** before **`Player says:`** (deliveries, suspicion, **Wife status**, apartments, **destination apartment** when leg active; **pickup location hidden** from player-facing orders). Also: use only apartment numbers named in CONTEXT; do not invent units; do not echo technical/placeholder wording from CONTEXT in replies.
...
| 2026-06-18 | **Post-meetup stability** — DDOL HUD merge + `DeliveryZone` persistent registration; main-menu retry fixes (LLM reconnect, drop points, stale toasts); timer **pauses** while PC open; messenger **bold H:** + reply normalize/strip; hostage pronoun guard; pause **Settings**; BPS Input System; objective under timer (**`refinements-changes.md`**) |
| 2026-06-18 | **Polish pass** — intro copy; computer arrow hints + hide top-left HUD at PC; delivery knock + walking loop SFX; `SceneNavigationUtility` for Main Menu load (**`refinements-changes.md`**, **`setup.md` §3c**) |
| 2026-06-18 | **Settings & audio** — mouse sens **0.1×–2×** + master SFX volume slider; door open/close SFX; editable pause settings panel (**`setup.md` §3d**, **`refinements-changes.md`**) |
